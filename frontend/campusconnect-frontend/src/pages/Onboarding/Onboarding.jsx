        ...
        <div style={{ padding: "2rem" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", marginBottom: "2rem" }}>
            {STEPS.map((s, i) => (
              <React.Fragment key={i}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", color: i <= step ? "var(--accent)" : "var(--text-muted)", fontSize: "0.75rem", fontWeight: i === step ? 700 : 500 }}>
                  <div style={{ width: "22px", height: "22px", borderRadius: "9999px", background: i < step ? "var(--accent)" : i === step ? "var(--accent-soft)" : "var(--bg-secondary)", border: `1px solid ${i <= step ? "var(--accent)" : "var(--border)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.65rem", fontWeight: 700, color: i < step ? "#0a0c14" : i === step ? "var(--accent)" : "var(--text-muted)" }}>
                    {i < step ? <Check size={11} /> : i + 1}
                  </div>
                  {s}
                </div>
                {i < STEPS.length - 1 && <div style={{ width: "24px", height: "1px", background: i < step ? "var(--accent)" : "var(--border)" }} />}
              </React.Fragment>
            ))}
          </div>
          ...
          {step === 0 && (
            <div style={{ textAlign: "center" }}>
              ...
              <h1 style={{ fontFamily: "Syne, sans-serif", fontSize: "1.75rem", fontWeight: 800, color: "var(--text-primary)", margin: "0 0 0.75rem" }}>Welcome to CampusConnect!</h1>
          ...
          {step === 1 && (
            <div>
              <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
                ...
                <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "1.5rem", fontWeight: 800, color: "var(--text-primary)", margin: "0 0 0.5rem" }}>What are you into?</h2>
                <p style={{ color: "var(--text-secondary)", fontSize: "0.875rem" }}>Pick your interests — we'll find the right mentors for you</p>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.625rem", marginBottom: "1.5rem" }}>
                {INTERESTS.map((interest) => {
                  ...
                  return (
                    <button key={interest.id} onClick={() => toggleInterest(interest.id)}
                      style={{ display: "flex", alignItems: "center", gap: "0.625rem", padding: "0.75rem 1rem", borderRadius: "0.75rem", textAlign: "left", background: selected ? `${interest.color}15` : "var(--bg-secondary)", border: `1px solid ${selected ? interest.color : "var(--border)"}`, color: selected ? interest.color : "var(--text-secondary)", cursor: "pointer", transition: "all 0.15s ease", fontFamily: "DM Sans, sans-serif", fontSize: "0.8125rem", fontWeight: 600 }}>
          ...
          {step === 2 && (
            <div>
              <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
                <div style={{ fontSize: "2rem", marginBottom: "0.75rem" }}>🚀</div>
                <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "1.5rem", fontWeight: 800, color: "var(--text-primary)", margin: "0 0 0.5rem" }}>What's your goal?</h2>
                <p style={{ color: "var(--text-secondary)", fontSize: "0.875rem" }}>This helps us personalize your experience</p>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem", marginBottom: "1.5rem" }}>
                {[
                  ...
                  <button key={g.id} onClick={() => setGoal(g.id)}
                    style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "0.875rem 1rem", borderRadius: "0.75rem", textAlign: "left", background: goal === g.id ? "var(--accent-soft)" : "var(--bg-secondary)", border: `1px solid ${goal === g.id ? "var(--accent)" : "var(--border)"}`, cursor: "pointer", transition: "all 0.15s ease", fontFamily: "DM Sans, sans-serif" }}>
              ...
              <div style={{ padding: "1rem", background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: "0.75rem", marginBottom: "1.5rem" }}>
                ...
                <div style={{ fontSize: "0.8125rem", color: "var(--text-secondary)" }}>Your profile is ready to go! You can update these anytime in Profile Settings.</div>
              </div>
              {error && (
